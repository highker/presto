        ...
        implements TestingHttpClient.Processor
{
            ...
            throws Exception
    {
        ...
        if (requestLocation.isSummaryOnly()) {
            ...
            return sizeResponse(requestLocation, maxSize);
        }
        assertEquals(maxSize, buffers.getUnchecked(requestLocation.getLocation()).getMaxSize());
        return pageResponse(requestLocation, maxSize);
    }

    private TestingResponse sizeResponse(RequestLocation requestLocation, DataSize maxSize)
    {
    ...
    private TestingResponse pageResponse(RequestLocation requestLocation, DataSize maxSize)
    {
    ...
    private static class MockBuffer
    {
        ...
        public BufferResult getPages(long sequenceId, DataSize maxSize)
        {
            ...
            long responseSize = serializedPage.getRetainedSizeInBytes();
            while (responseSize < maxSize.toBytes()) {
                ...
                responsePages.add(serializedPage);
                responseSize += serializedPage.getRetainedSizeInBytes();
            }
        ...
        public BufferSummary getSummary(long sequenceId, DataSize maxSize)
        {
            ...
            for (SerializedPage page : serializedPages) {
                if (responseSize > maxSize.toBytes()) {
                    break;
                }
                responseSizesInBytes.add((long) page.getRetainedSizeInBytes());
                responseSize += page.getRetainedSizeInBytes();
            }
