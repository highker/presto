...
public class TestTaskThriftIntegration
{
    ...
    public void testServer()
    {
        ...
        try (DriftNettyMethodInvokerFactory<?> invokerFactory = createStaticDriftNettyMethodInvokerFactory(new DriftNettyClientConfig())) {
            ...
            TaskThriftClient client = clientFactory.createDriftClient(TaskThriftClient.class).get();
            ...
            implements Module
    {
        ...
        {
            binder.bind(TaskThriftResource.class).in(Scopes.SINGLETON);

            driftServerBinder(binder).bindService(TaskThriftResource.class);
        }
        ...
        @Singleton
        @ForAsyncHttp
        public static BoundedExecutor createAsyncHttpResponseExecutor(@ForAsyncHttp ExecutorService coreExecutor)
        {
        ...
        public static TaskManager createTaskManager()
        {
            return new TaskManager() {
                ...
                @Override
                public TaskInfo updateTask(Session session, TaskId taskId, Optional<PlanFragment> fragment, List<TaskSource> sources, OutputBuffers outputBuffers, OptionalInt totalPartitions, Optional<TableWriteInfo> tableWriteInfo)
                {
                    throw new UnsupportedOperationException();
                }
