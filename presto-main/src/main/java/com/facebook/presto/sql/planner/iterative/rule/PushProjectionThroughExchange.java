        ...
        implements Rule<ProjectNode>
{
    ...
    public Result apply(ProjectNode project, Captures captures, Context context)
    {
        ...
        for (int i = 0; i < exchange.getSources().size(); i++) {
            ...
            if (exchange.getPartitioningScheme().getHashColumn().isPresent()) {
                ...
                projections.put(exchange.getPartitioningScheme().getHashColumn().get(), new SymbolReference(exchange.getPartitioningScheme().getHashColumn().get().getName()));
    ...
    private static Map<Symbol, SymbolReference> extractExchangeOutputToInput(ExchangeNode exchange, int sourceIndex)
    {
        ...
        for (int i = 0; i < exchange.getOutputSymbols().size(); i++) {
            outputToInputMap.put(exchange.getOutputSymbols().get(i), new SymbolReference(exchange.getInputs().get(sourceIndex).get(i).getName()));
        }
